...
HEROKU_PROJECT_NAME = "flask-backend-ee461l"
os.environ["SERVER_HOST"] = "https://{}.herokuapp.com/".format(HEROKU_PROJECT_NAME)
...
from Database.Hardware_Sets.hardware_set_service import HardwareSetService
...
def create_app() -> Flask:
    ...
    my_hardware_set_service_g = HardwareSetService()
    ...
    # Api route for a hardware_set
    # Forms mean that the user and pass aren't exposed in the request url
    @app.route("/api/data_set/find", methods=["GET", "POST"])
    def get_data_set():
        data = request.get_json(force = True)
        print("Request method : {} of type {}".format(request.method, type(request.method)))
        print("Got body content : {} of type {}".format(data, type(data)))
        if (request.method == "POST") and ("set_name" in data):
            this_set_name = data["set_name"]
            print("Trying to retrieve data set : {}".format(this_set_name))
            this_data_set = my_data_set_service_g.find_data_set(this_set_name)
            print("Returning data set : {}".format(this_data_set))
            return jsonify(this_data_set)
        else:
            fake_dict = {
                "data_set_name" : "No Name", 
                "file_size": "0 B",
                "description": "No desciption.",
                "data_set_url": "No URL"
            }
            print("Bad request, returning hardware set : {}".format(fake_dict))
    ...
    # Grabs all the not private data_sets names field
    @app.route("/api/data_set/find_all", methods=["GET"])
    def find_all_public_data_sets():
        list_of_set_names = my_data_set_service_g.find_all_public_data_sets()
        return jsonify(list_of_set_names)
    ...
    # Grabs all the not hardware_sets names field
    @app.route("/api/hardware_set/find_all", methods=["GET"])
    def find_all_hardware_sets():
        list_of_set_names = my_hardware_set_service_g.find_all_hardware_sets()
        return jsonify(list_of_set_names)

    # Api route for a hardware_set
    # Forms mean that the user and pass aren't exposed in the request url
    @app.route("/api/hardware_set/find", methods=["GET", "POST"])
    def get_hardware_set():
        ...
        if (request.method == "POST") and ("set_name" in data):
            this_set_name = data["set_name"]
            print("Trying to retrieve hardware set : {}".format(this_set_name))
            this_hardware_set = my_hardware_set_service_g.find_hardware_set(this_set_name)
            print("Returning hardware set : {}".format(this_hardware_set))
            return jsonify(this_hardware_set)
        else:
            fake_dict = {
    ...
    # Api for checking out hardware resources
    @app.route("/api/hardware_set_tickets/find_for", methods=["GET", "POST"])
    #@login_required
    def find_for_ticket_api():
        data = request.get_json(force = True)
        print("Request method : {} of type {}".format(request.method, type(request.method)))
        print("Got body content : {} of type {}".format(data, type(data)))
        if (request.method == "POST"):
            this_user_name = data["user_name"]
            #this_user_name = my_login_set_service_g.get_user_name_by_id(current_user.id)
            print("Trying to create hardware set ticket for : {}".format(this_user_name))
            result = my_hardware_set_service_g.find_tickets_for(this_user_name)
            print("Result : {}".format(result))
            return jsonify(result)
        else:
            print("Bad request, returning []")
            return jsonify([])

    # Api for checking out hardware resources
    @app.route("/api/hardware_set_tickets/checkout", methods=["GET", "POST"])
    #@login_required
    def create_ticket_api():
        ...
        if (request.method == "POST"):
            this_set_name = data["set_name"]
            this_unit_amount = data["unit_amount"]
            this_user_name = data["user_name"]
            #this_user_name = my_login_set_service_g.get_user_name_by_id(current_user.id)
            print("Trying to create hardware set ticket for : {}".format(this_user_name))
            result = my_hardware_set_service_g.create_ticket(this_set_name, this_unit_amount, this_user_name)
            print("Result : {}".format(result))
            return jsonify(result)
        else:
            print("Bad request, returning False")
            return jsonify(False)

    # Api for checking out hardware resources
    @app.route("/api/hardware_set_tickets/update", methods=["GET", "POST"])
    #@login_required
    def update_ticket_api():
        ...
        if (request.method == "POST"):
            ...
            this_user_name = data["user_name"]
            #this_user_name = my_login_set_service_g.get_user_name_by_id(current_user.id)
            print("Trying to update hardware set ticket for : {}".format(this_user_name))
            result = my_hardware_set_service_g.update_ticket(this_set_name, this_unit_amount, this_user_name)
            print("Result : {}".format(result))
            return jsonify(result)
        else:
            print("Bad request, returning False")
    ...
    # Api for checking out hardware resources
    @app.route("/api/hardware_set_tickets/delete", methods=["GET", "POST"])
    #@login_required
    def delete_ticket_api():
        ...
        if (request.method == "POST"):
            ...
            this_user_name = data["user_name"]
            #this_user_name = my_login_set_service_g.get_user_name_by_id(current_user.id)
            print("Trying to delete hardware set ticket for : {}".format(this_user_name))
            result = my_hardware_set_service_g.delete_ticket(this_set_name, this_user_name)
            print("Result : {}".format(result))
            return jsonify(result)
        else:
            print("Bad request, returning False")
    ...
    def login():
        ...
        print("Got body content : {} of type {}".format(form, type(form)))
        if (request.method == "POST") and ("user_name" in form) and ("user_password" in form):
            ...
            if my_login_set_service_g.validate_login_set(this_user_name, this_user_password):
                ...
                remember = form.get("remember", "yes") == "yes"
                ...
                if this_user_id:
                    ...
                    if login_user(user_model, remember=remember):
                        ...
                        token = generate_csrf()
                        print("Success, sending : {}".format(token))
    ...
    # Api route for a signup
    # Forms mean that the user and pass aren't exposed in the request url
    @app.route("/api/signup", methods=["GET", "POST"])
    def signup():
        form = request.get_json(force = True)
        print("Got body content : {} of type {}".format(form, type(form)))
        if (request.method == "POST") and ("user_name" in form) and ("user_password" in form) and ("user_email" in form):
            #print("route create")
            #print(request.url)
            # Get the request user_name arg
            this_user_name = form.get("user_name")
            #print(this_user_name)
            # Get the request user_password arg
            this_user_password = form.get("user_password")
            # Get the request user_email arg
            this_user_email = form.get("user_email")
            status = my_login_set_service_g.create_login_set_for(this_user_name, this_user_password, this_user_email)
            if status == True:
                try:
                    # Check if the user should be remembered
                    remember = form.get("remember", "yes") == "yes"
            ...
            print("Malformed content or request")
            return jsonify({"token": None})
